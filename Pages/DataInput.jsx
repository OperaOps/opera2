...
import { DataUpload } from "@/entities/DataUpload";
  ...
  const handleFileUpload = useCallback(async (files) => {
    ...
    for (const file of files) {
      try {
        ...
        const uploadRecord = await DataUpload.create({
          file_name: file.name,
          file_url,
        ...
        if (extractResult.status === 'success' && extractResult.output) {
          ...
          await entityType.bulkCreate(dataArray);
          
          await DataUpload.update(uploadRecord.id, {
            processed: true,
            processing_status: 'completed',
            records_count: dataArray.length
          });

          setUploadedFiles(prev => prev.map(f => f.file_name === file.name ? { ...uploadRecord, processing_status: 'completed', records_count: dataArray.length } : f));
        } else {
