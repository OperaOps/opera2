...
const OUTPUT_FILE = 'greyfinchprocedures.txt';
const CHECKPOINT_FILE = 'checkpoint-procedures.json';

// GraphQL query for procedures (based on the treatments route)
const PROCEDURES_QUERY = `
  query getProcedures($limit: Int, $offset: Int) {
    procedures(limit: $limit, offset: $offset) {
      id
      procedureDate
      procedureType {
        name
...
function formatProcedureRecord(record: any): string {
  // Extract key information and create compact JSON
  const compactData = {
    id: record.id,
    procedureDate: record.procedureDate || 'Unknown',
  ...
  return `procedures\t${record.id}\t${record.procedureDate || 'Unknown'}\t${truncatedData}`;
}

async function exportProceduresBulletproof(): Promise<void> {
  console.log('🚀 Starting bulletproof procedures export...');
  ...
  try {
    while (true) {
      console.log(`\n  📄 BATCH ${batchNumber}: Fetching procedures`);
      console.log(`     Offset: ${offset}, Limit: 20`);
      
      const response = await makeRequestWithRetry(PROCEDURES_QUERY, {
        limit: 20,
        offset: offset
      });
      
      const records = response.data.procedures || [];
      ...
      for (const record of records) {
        if (!uniqueRecords.has(record.id)) {
          ...
          const formattedRecord = formatProcedureRecord(record);
    ...
    console.log(`📊 Total unique procedures exported: ${totalRecords}`);
    console.log(`📅 Date range: ${firstDate || 'Unknown'} to ${lastDate || 'Unknown'}`);
    
  } catch (error: any) {
    console.error(`❌ Error fetching procedures batch ${batchNumber}:`, error.message);
...
async function main() {
  try {
    await exportProceduresBulletproof();
  } catch (error: any) {
