...
// Simple GraphQL query for patients
...
function formatPatientRecord(record: any): string {
  // Simple formatting without complex nested objects
  const firstName = record.person?.firstName || 'Unknown';
  const lastName = record.person?.lastName || 'Unknown';
  const email = record.person?.email || 'Unknown';
  const phoneNumber = record.person?.phoneNumber || 'Unknown';
  const dateOfBirth = record.person?.dateOfBirth || 'Unknown';
  const gender = record.person?.gender || 'Unknown';
  ...
  const jsonData = JSON.stringify(simpleData);
  
  return `patients\t${record.id}\t${record.createdAt || 'Unknown'}\t${jsonData}`;
}
