...
/**
 * Greyfinch Data Export - Working Version
 * 
 * Exports ALL data starting from the beginning (like the 1580 record success).
 * Writes each batch immediately to prevent data loss.
 * Shows date checks in terminal for verification.
 * 
 * Usage:
 * npx tsx scripts/working-export-final.ts
 */
...
function loadCheckpoint(entityName: string): Checkpoint {
  const checkpointFile = `checkpoint-final-${entityName}.json`;
...
function saveCheckpoint(checkpoint: Checkpoint): void {
  const checkpointFile = `checkpoint-final-${checkpoint.entityName}.json`;
  fs.writeFileSync(checkpointFile, JSON.stringify(checkpoint, null, 2));
}

async function exportEntityWorking(entityName: string, query: string, outputStream: fs.WriteStream): Promise<number> {
  console.log(`📊 Exporting ${entityName} data (like the 1580 record success)...`);
  ...
  while (hasMore) {
    ...
    try {
      ...
      if (batchDates.length > 0) {
        ...
        console.log(`  📅 Sample dates in batch: ${batchDates.slice(0, 3).join(', ')}${batchDates.length > 3 ? '...' : ''}`);
      }
...
async function exportWorkingData() {
  console.log('🚀 Starting Greyfinch Data Export - Working Version...');
  console.log('📝 This will pull ALL data like the successful 1580 record export\n');
  
  const outputFile = 'greyfinchactualfinal.txt';
  
  // Write header
  const outputStream = fs.createWriteStream(outputFile, { flags: 'w' });
  outputStream.write('# Greyfinch Data Export - Working Version\n');
  outputStream.write('# Format: Entity\tID\tCreatedAt\tJSON\n');
  outputStream.write(`# Generated: ${new Date().toISOString()}\n`);
  outputStream.write('# This export pulls ALL data like the successful 1580 record export\n\n');
  outputStream.end();
  ...
  try {
    ...
    totalRecords += await exportEntityWorking('appointmentBookings', appointmentQuery, appointmentStream);
    ...
    totalRecords += await exportEntityWorking('patients', patientQuery, patientStream);
    patientStream.end();
    
    console.log('\n🎉 Working data export completed!');
