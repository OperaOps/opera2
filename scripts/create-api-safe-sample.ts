...
const dataFiles: DataFile[] = [
  ...
  },
  {
    ...
    sampleSize: 2000 // Reduced for API limits
  }
];
...
async function createAPISafeSample(): Promise<string> {
  console.log('🔄 Creating API-safe RAG sample (optimized for Gemini quota limits)...');
  
  let combinedData = '';
  let totalRecords = 0;
  
  // Add header information
  combinedData += '# Greyfinch API-Safe RAG Sample\n';
  combinedData += `# Generated: ${new Date().toISOString()}\n`;
  combinedData += '# This file is optimized for Gemini API quota limits\n';
  combinedData += '# Balanced for comprehensive coverage while staying within free tier limits\n\n';
  ...
  combinedData += `# Generated: ${new Date().toISOString()}\n`;
  combinedData += `# This API-safe sample stays within Gemini free tier limits\n`;
...
async function main() {
  try {
    const apiSafeSample = await createAPISafeSample();
    
    const outputPath = '/Users/anishsuvarna/Downloads/opera copy/greyfinch-api-safe-rag.txt';
    fs.writeFileSync(outputPath, apiSafeSample, 'utf8');
    
    const fileSize = (fs.statSync(outputPath).size / 1024 / 1024).toFixed(2);
    
    console.log('\n🎉 Successfully created API-safe RAG sample!');
    console.log(`📁 Output file: ${outputPath}`);
    console.log(`📊 File size: ${fileSize} MB`);
    console.log(`📈 Total sampled records: ${apiSafeSample.split('\n').filter(line => 
      line.trim() && 
      !line.startsWith('#') && 
      !line.startsWith('Format:') &&
      !line.startsWith('Generated:') &&
    ...
    console.log('\n📋 Preview of API-safe sample:');
    console.log(apiSafeSample.split('\n').slice(0, 20).join('\n'));
