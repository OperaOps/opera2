...
const dataFiles: DataFile[] = [
  ...
  },
  {
    ...
    sampleSize: 3000 // Sample from treatments
  }
];
...
async function createSmartRAGSample(): Promise<string> {
  console.log('🔄 Creating smart RAG sample from all Greyfinch data files...');
  
  let combinedData = '';
  let totalRecords = 0;
  
  // Add header information
  combinedData += '# Greyfinch Smart RAG Sample\n';
  combinedData += `# Generated: ${new Date().toISOString()}\n`;
  combinedData += '# This file contains a strategic sample of ALL available Greyfinch data for AI analysis\n';
  combinedData += '# Optimized for token limits while maintaining data diversity\n\n';
  ...
  combinedData += `# Generated: ${new Date().toISOString()}\n`;
  combinedData += `# This sample provides comprehensive coverage while staying within AI token limits\n`;
...
async function main() {
  try {
    const smartSample = await createSmartRAGSample();
    
    const outputPath = '/Users/anishsuvarna/Downloads/opera copy/greyfinch-smart-rag.txt';
    fs.writeFileSync(outputPath, smartSample, 'utf8');
    
    const fileSize = (fs.statSync(outputPath).size / 1024 / 1024).toFixed(2);
    
    console.log('\n🎉 Successfully created smart RAG sample!');
    console.log(`📁 Output file: ${outputPath}`);
    console.log(`📊 File size: ${fileSize} MB`);
    console.log(`📈 Total sampled records: ${smartSample.split('\n').filter(line => 
      line.trim() && 
      !line.startsWith('#') && 
      !line.startsWith('Format:') &&
      !line.startsWith('Generated:') &&
    ...
    console.log('\n📋 Preview of smart sample:');
    console.log(smartSample.split('\n').slice(0, 25).join('\n'));
