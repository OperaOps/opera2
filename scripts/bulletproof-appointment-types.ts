...
const OUTPUT_FILE = 'greyfinchapptypes.txt';
const CHECKPOINT_FILE = 'checkpoint-appointment-types.json';

// GraphQL query for appointment types
const APPOINTMENT_TYPES_QUERY = `
  query getAppointmentTypes($limit: Int, $offset: Int) {
    appointmentTypes(limit: $limit, offset: $offset) {
      id
      name
      category
      duration
...
function formatAppointmentTypeRecord(record: any): string {
  ...
  return `appointmentTypes\t${record.id}\t${record.name || 'Unknown'}\t${truncatedData}`;
}

async function exportAppointmentTypesBulletproof(): Promise<void> {
  console.log('🚀 Starting bulletproof appointment types export...');
  ...
  if (fs.existsSync(OUTPUT_FILE)) {
    try {
      ...
      for (const line of lines) {
        if (line.startsWith('appointmentTypes\t')) {
  ...
  try {
    while (true) {
      console.log(`\n  📄 BATCH ${batchNumber}: Fetching appointment types`);
      console.log(`     Offset: ${offset}, Limit: 20`);
      
      const response = await makeRequestWithRetry(APPOINTMENT_TYPES_QUERY, {
        limit: 20,
        offset: offset
      });
      
      const records = response.data.appointmentTypes || [];
      ...
      // Track date ranges (for appointment types, we'll use name as a simple identifier)
      ...
      for (const record of records) {
        if (!uniqueRecords.has(record.id)) {
          ...
          const formattedRecord = formatAppointmentTypeRecord(record);
    ...
    console.log(`📊 Total unique appointment types exported: ${totalRecords}`);
    console.log(`📅 Name range: ${firstDate || 'Unknown'} to ${lastDate || 'Unknown'}`);
    
  } catch (error: any) {
    console.error(`❌ Error fetching appointment types batch ${batchNumber}:`, error.message);
...
async function main() {
  try {
    await exportAppointmentTypesBulletproof();
  } catch (error: any) {
