...
async function createMiniRAGSample(): Promise<string> {
  console.log('🔄 Creating mini RAG sample for testing...');
  
  let combinedData = '';
  let totalRecords = 0;
  
  // Add header information
  combinedData += '# Greyfinch Mini RAG Sample (For Testing)\n';
  combinedData += `# Generated: ${new Date().toISOString()}\n`;
  combinedData += '# This file contains a small sample of Greyfinch data for AI testing\n';
  combinedData += '# Optimized for quick testing and development\n\n';
  ...
  combinedData += `# Generated: ${new Date().toISOString()}\n`;
  combinedData += `# This mini sample is perfect for testing and development\n`;
...
async function main() {
  try {
    const miniSample = await createMiniRAGSample();
    
    const outputPath = '/Users/anishsuvarna/Downloads/opera copy/greyfinch-mini-rag.txt';
    fs.writeFileSync(outputPath, miniSample, 'utf8');
    
    const fileSize = (fs.statSync(outputPath).size / 1024).toFixed(2);
    
    console.log('\n🎉 Successfully created mini RAG sample!');
    console.log(`📁 Output file: ${outputPath}`);
    console.log(`📊 File size: ${fileSize} KB`);
    console.log(`📈 Total sampled records: ${miniSample.split('\n').filter(line => 
      line.trim() && 
      !line.startsWith('#') && 
      !line.startsWith('Format:') &&
      !line.startsWith('Generated:') &&
    ...
    console.log('\n📋 Preview of mini sample:');
    console.log(miniSample.split('\n').slice(0, 20).join('\n'));
