...
const OUTPUT_FILE = path.resolve(__dirname, 'recent-appointment-types-2024-2025.txt');
const CHECKPOINT_FILE = path.resolve(__dirname, 'checkpoint-recent-appointment-types.json');
...
function formatAppointmentTypeRecord(record: any): string {
  ...
  const name = record.name || 'Unknown Service';
...
async function exportRecentAppointmentTypes() {
  console.log('🚀 Starting recent appointment types export (2024-2025)...');
  ...
  try {
    while (hasMoreData) {
      ...
      try {
        ...
        console.log(`📋 Found ${appointmentTypes.length} appointment types in this batch`);

        // Process and write records
        for (const appointmentType of appointmentTypes) {
          const formattedRecord = formatAppointmentTypeRecord(appointmentType);
        ...
        if (appointmentTypes.length > 0) {
          const sampleRecord = formatAppointmentTypeRecord(appointmentTypes[0]);
        ...
        if (appointmentTypes.length < PAGE_SIZE) {
...
// Run the export
exportRecentAppointmentTypes().catch(console.error);
