...
const OUTPUT_FILE = path.resolve(__dirname, 'recent-patients-2024-2025.txt');
const CHECKPOINT_FILE = path.resolve(__dirname, 'checkpoint-recent-patients.json');
...
function formatPatientRecord(record: any): string {
  ...
  return `Patient: ${firstName} ${lastName} (ID: ${id}) - Created: ${createdAt}`;
}
...
async function exportRecentPatients() {
  console.log('🚀 Starting recent patients export (2024-2025)...');
  ...
  // GraphQL query for recent patients (2024-2025)
  const query = `
    query GetRecentPatients($limit: Int!, $offset: Int!) {
      patients(
        limit: $limit
        offset: $offset
        where: {
  ...
  try {
    while (hasMoreData) {
      ...
      try {
        ...
        console.log(`📋 Found ${patients.length} patients in this batch`);

        // Process and write records
        for (const patient of patients) {
          const formattedRecord = formatPatientRecord(patient);
        ...
        if (patients.length > 0) {
          const sampleRecord = formatPatientRecord(patients[0]);
...
// Run the export
exportRecentPatients().catch(console.error);
