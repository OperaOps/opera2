...
const CONTENT_MODES: { value: ContentMode; label: string; desc: string }[] = [
  { value: "template", label: "Template", desc: "Fastest, uses proven scripts" },
  { value: "template_ai", label: "Template + AI", desc: "Personalized with AI" },
  { value: "full_ai", label: "Full AI", desc: "Fully custom generation" },
];
            ...
            {formStep === 1 && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5 shadow-sm">
                  ...
                  <FormField label="Content Mode" hint="Template is fastest. Full AI creates a fully custom script.">
                    <div className="grid grid-cols-3 gap-2">
                      {CONTENT_MODES.map((m) => (
                        ...
                        >
                          <p className={`text-sm font-medium ${contentMode === m.value ? "text-purple-700" : "text-gray-700"}`}>{m.label}</p>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
                    ...
                    <div><span className="text-gray-400">Content Mode</span><p className="font-medium text-gray-900 mt-0.5">{CONTENT_MODES.find((m) => m.value === contentMode)?.label}</p></div>
...
function GeneratingScreen({ jobStatus, onCancel }: { jobStatus: JobStatus | null; onCancel: () => void }) {
...
export default function VideoPrototypePage() {
    ...
    <AnimatePresence mode="wait">
      ...
      {step === "form" && (
        <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <FormWizard onSubmit={handleSubmit} onBack={() => setStep("start")} />
        </motion.div>
      )}
      {step === "generating" && (
        <motion.div key="generating" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <GeneratingScreen jobStatus={jobStatus} onCancel={handleReset} />
        </motion.div>
      )}
      {step === "preview" && jobStatus && (
        <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <PreviewScreen jobStatus={jobStatus} onNewVideo={handleReset} onEdit={() => setStep("form")} />
        </motion.div>
      )}
    </AnimatePresence>
  );
